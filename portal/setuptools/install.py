...
class extend_install(distutils.commands.install):

  def __init__(self):
    distutils.commands.__init__(self)
    ##self.scripts_dir = ? ## is it set?

  def run(self):
    super(django_scripts, self).run()
    log.info("####### Dumping you into prompt!!!")
    import code
    code.interact(locals=locals())

  def create_django_scripts(self, dist):
  ...
  def write_script(self, script_name, contents):
    log.info("Installing the Django script %s to %s", script_name, 
        self.script_dir)
  ...
  def install_django_scripts(self, dist):
